...
! ---- in/outs
            ...
            lflx, gflx, zorlo, ustar, qsfc, ch, cm, chh, cmm,&
            rb_wat, stress_wat, ffmm_wat, ffhh_wat, fm10_wat,&
            fh2_wat, tprcp_wat, ztmax_wat,                   &
            h_ML, t_wML, t_mnw, H_B, T_B, t_bot1,            &
            t_bot2, c_t, T_snow, T_ice, tsurf_ice,           &
            errmsg, errflg                     )     
!zorlo(i)      surface_roughness_length_over_water 
!cd_wat(i)     surface_drag_coefficient_for_momentum_in_air_over_water
!cdq_wat(i)    surface_drag_coefficient_for_heat_and_moisture_in_air_over_water                
!rb_wat(i)     bulk_richardson_number_at_lowest_model_level_over_water         
!stress_wat(i) surface_wind_stress_over_water          
!ffmm_wat(i)   Monin_Obukhov_similarity_function_for_momentum_over_water 
!ffhh_wat(i)   Monin_Obukhov_similarity_function_for_heat_over_water
!*uustar_wat(i) surface_friction_velocity_over_water
!fm10_wat(i)   Monin_Obukhov_similarity_function_for_momentum_at_10m_over_water 
!fh2_wat(i)    Monin_Obukhov_similarity_function_for_heat_at_2m_over_water
!*tsfc_wat(i)   surface_skin_temperature_over_water
!cmm_wat(i)    surface_drag_wind_speed_for_momentum_in_air_over_water
!chh_wat(i)    surface_drag_mass_flux_for_heat_and_moisture_in_air_over_water 
!*gflx_wat(i)   upward_heat_flux_in_soil_over_water
!*ep1d_wat(i)   surface_upward_potential_latent_heat_flux_over_water (wm-2)       
!*evap_wat(i)   kinematic_surface_upward_latent_heat_flux_over_water (kgkg-1 m s-1)
!*hflx_wat(i)   kinematic_surface_upward_sensible_heat_flux_over_water (K m s-1) 
!qss_wat(i)    surface_specific_humidity_over_water (kg kg-1)
...
!     integer, dimension(im), intent(in) :: islmsk
     ...
     &           rb_wat, stress_wat, ffmm_wat, ffhh_wat, fm10_wat,            &
     &           fh2_wat, tprcp_wat, ztmax_wat, zorlo
...
!  --- ...  set flag for lake points
      ...
      do_flake = .false.
      do i = 1, im
            flag(i) = flag_iter(i) .and. use_flake(i) > 0 
...
!         w_extinc(i) = 3.0
     ...
     write(0,1005) rb_wat(i), stress_wat(i), ffmm_wat(i), ffhh_wat(i), fm10_wat(i)
     write(0,1006) fh2_wat(i), tprcp_wat(i), ztmax_wat(i), zorlo(i)
 ...
 1005 format ( 'rb_wat=',5(F15.4,1x))
 1006 format ( 'fh2_wat=',4(F16.4,1x))
!
!  call lake interface
       do i=1,im
         if (flag(i) .and. lakedepth(i) > 1.0) then
...
!
                  T_bot_2_in, T_bot_2_out,u_star, q_sfc,chh_out,cmm_out,zorlo_out ) 
