...
!!
      subroutine noahmpdrv_init(im,lsm, lsm_noahmp, me, isot, ivegsrc, &
        ...
        use noahmp_tables,     only : smcdry_table,smcmax_table
...
! --      for zsin to zsout interpolation - hard wired from 4 -> 9 levels for now
       ...
       &       'LSM is different from Noah MP'
        ...
        if (lsoil /= 4) then
          errmsg = 'This version of NOAH MP LSM expects IC from Noah with 4 soil layers '// &
                   'namelist parameter is not 4. Exiting...'
          errflg = 1
          return
        end if
        ...
        ! -- call to init 9-layer Noah MP soil
...
! data zsoil  / -0.02, -0.06, -0.14, -0.46, -0.74, -1.26, -1.94, -4.06 /
! data zsoil  / 0.00, -0.02, -0.06, -0.14, -0.46, -0.74, -1.26, -1.94, -4.06 /
...
!     integer :: i,j,k,l,ii,jj,num_soil_layers
      ...
      ! Noah lsm input
         ...
         do i = 1,slcats
           ...
           WLTSMC1   = MAXSMCnoah(I) *                       &
           (200.0/SATPSInoah(I))**(-1.0/BBnoah(I))
